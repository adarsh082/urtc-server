...
func RequestCollaboration(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writePluginError(w, http.StatusNotFound, "Owner not found")
		return
	}
	...
	if err != nil {
		writePluginError(w, http.StatusBadRequest, "Invalid project ID")
		return
	}

	project, err := projectModel.GetProjectByID(projectID)
	if err != nil || project.OwnerID != owner.ID {
		writePluginError(w, http.StatusForbidden, "Project not found or access denied")
		return
	}
	...
	if err != nil {
		if err == sql.ErrNoRows {
			// Create a placeholder user for the collaborator
			// This allows adding people who haven't signed in yet
			placeholderUsername := "user_" + req.CollaboratorEmail
			if idx := strings.Index(req.CollaboratorEmail, "@"); idx != -1 {
				placeholderUsername = req.CollaboratorEmail[:idx]
			}
			
			collaborator, err = userModel.CreateUser(0, placeholderUsername, req.CollaboratorEmail)
			if err != nil {
				writePluginError(w, http.StatusInternalServerError, "Failed to create placeholder user: "+err.Error())
				return
			}
			fmt.Printf("Created placeholder user for collaborator: %s\n", req.CollaboratorEmail)
		} else {
			writePluginError(w, http.StatusInternalServerError, "Database error: "+err.Error())
			return
		}
	}

	collabModel := &db.CollaboratorModel{DB: db.DB}
	if existing, err := collabModel.GetCollaborationByUserAndProject(collaborator.ID, projectID); err == nil {
		writePluginJSON(w, http.StatusOK, CollaborationResponse{
			Success:   true,
			Message:   "Collaboration request already exists",
			CollabID:  existing.ID.String(),
			Token:     existing.ID.String(),
			ProjectID: project.ID.String(),
		})
		return
	} else if err != sql.ErrNoRows {
		writePluginError(w, http.StatusInternalServerError, "Failed to check collaboration request")
		return
	}

	collab, err := collabModel.CreateCollaboration(collaborator.ID, projectID, "pending")
	if err != nil {
		writePluginError(w, http.StatusInternalServerError, "Failed to create collaboration request: "+err.Error())
		return
	}
